class PersonAdvSearchResult extends React.Component {
    ...
    render() {
        return (<li>
            <h2><a class="link-people" href={`/people/${this.props.person_id}`}>{this.props.person_full_name}</a></h2>
...
class PersonAdvancedSearch extends React.Component {
    ...
    sendSearch(){
        ...
        let requestUrl = (`${this.state.origin}/api/people/${this.createSearchQueryString(searchQuery)}`);
        ...
        fetch(requestUrl,{
            ...
            history.pushState(this.state.query,
                "Person Search Results - Access Missouri",
                `/search/people/${this.createSearchQueryString(this.state.query)}`);
        });
